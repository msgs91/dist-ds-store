...
public class TwoPhaseCommitStrategy implements ReplicationStrategy {
    ...
    try {
      ...
      switch(type) {
        ...
        case "commit":
          ...
          log.info(String.format("Putting next txn id to %d", packet.getTransactionId()+1));
  ...
  public synchronized void processWalPacket(Packet packet) {
    ...
    switch(type) {
      case "prepare":
        log.info("Reading prepare for transaction id %d ", packet.getTransactionId());
        store.setNextTransactionId(packet.getTransactionId()+1);
        break;
      case "commit":
        try {
  ...
  private int prepare(Long transactionId) {
    ...
    return replicator.waitFor(replicaResponses);
  }
  
  private int commit(Packet packet) {
    return replicator.waitFor(replicator.sendToReplicas(packet));
  }
}
